...
class Fields(Enum):
    ...
    TWEET = "created_at,public_metrics,lang,author_id,referenced_tweets,in_reply_to_user_id"
...
class TwitterApiServiceV2():
    ...
    def get_public_list_users(self, list_id: str, max_results: int = 100) -> list[User]:
        members = []
        ...
        # 全てのキーにuser_のプレフィックスをつける
        result = [{f"user_{k}": v for k, v in tweet.items()} for tweet in result]
        # Userオブジェクトに変換
        result = [User(**user) for user in result]
        return result
    

    def get_public_list_timeline(self, list_id: str, max_results: int = 100) -> list[Tweet]:
        tweets = []
        pagination_token = None
        try:
        ...
        result = [{f"tweet_{k}": v for k, v in tweet.items()} for tweet in result]
        # Twitterオブジェクトに変換
        result = [Tweet(**tweet) for tweet in result]
        return result
    ...
    def extract_tweet_data(tweets: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for tweet in tweets:
            ...
            if (referenced_tweets := tweet.get('referenced_tweets')) is not None:
                ...
                if (new_tweet_type := match_referenced_tweets(referenced_tweets)) is not None:
