...
class CRUDTwitter():
    ...
    async def get_tweets(
        self, first_tweet_created_at: str = None, session: AsyncSession = Depends(get_async_session)
    ) -> List[Tweet]:
        """ツイートを取得する"""
    ...
    async def save_tweets(
        self, tweets: list[Tweet], session: AsyncSession = Depends(get_async_session) 
    ) -> None:
        """ツイートを保存する"""
        ...
        # 既存のデータを取得
        tweets_in_database = await self.get_tweets(first_tweet_created_at=first_tweet_created_at, session=session)
        ...
        # 更新、追加、削除するデータを分類
        to_insert = [tweet for tweet in tweets if tweet.tweet_id not in tweet_ids_in_database]
        to_update = [tweet for tweet in tweets if tweet.tweet_id in tweet_ids_in_database and tweets_in_database_dict[tweet.tweet_id] != tweet]
        to_delete = [tweet for tweet in tweets_in_database if tweet.tweet_id not in tweet_id_set]
